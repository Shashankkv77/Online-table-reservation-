<?php<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Your Menu</title>
    <!-- Link to your CSS file -->
    <link rel="stylesheet" href="css/styles.css">
</head>
<body>
 echo "<section class='section' id='offers'>
 <div class='container'>
     <div class='row'>
         <div class='col-lg-4 offset-lg-4 text-center'>
             <div class='section-heading'>
                 <h6>Klassy Week</h6>
                 <h2>This Week’s Special Meal Offers</h2>
             </div>
         </div>
     </div>
     <div class='row'>
         <div class='col-lg-12'>
             <div class='row' id='tabs'>
                 <div class='col-lg-12'>
                     <div class='heading-tabs'>
                         <div class='row'>
                             <div class='col-lg-6 offset-lg-3'>
                                 <ul>
                                   <li><a href='#tabs-1'><img src='assets/images/tab-icon-01.png' alt=''>Breakfast</a></li>
                                   <li><a href='#tabs-2'><img src='assets/images/tab-icon-02.png' alt=''>Lunch</a></a></li>
                                   <li><a href='#tabs-3'><img src='assets/images/tab-icon-03.png' alt=''>Dinner</a></a></li>
                                 </ul>
                             </div>
                         </div>
                     </div>
                 </div>
                 <div class='col-lg-12'>
                     <section class='tabs-content'>
                         <article id='tabs-1'>
                             <div class='row'>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='left-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-01.png' alt=''>
                                                     <h4>Fresh Chicken Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>100 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-02.png' alt=''>
                                                     <h4>Orange Juice</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>85 rs.</h6>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-03.png' alt=''>
                                                     <h4>Fruit Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>90 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='right-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-04.png' alt=''>
                                                     <h4>Eggs Omelette</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>65 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-05.png' alt=''>
                                                     <h4>Dollma Pire</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>50 rs</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>

<?php
 echo "<section class='section' id='offers'>
 <div class='container'>
     <div class='row'>
         <div class='col-lg-4 offset-lg-4 text-center'>
             <div class='section-heading'>
                 <h6>Klassy Week</h6>
                 <h2>This Week’s Special Meal Offers</h2>
             </div>
         </div>
     </div>
     <div class='row'>
         <div class='col-lg-12'>
             <div class='row' id='tabs'>
                 <div class='col-lg-12'>
                     <div class='heading-tabs'>
                         <div class='row'>
                             <div class='col-lg-6 offset-lg-3'>
                                 <ul>
                                   <li><a href='#tabs-1'><img src='assets/images/tab-icon-01.png' alt=''>Breakfast</a></li>
                                   <li><a href='#tabs-2'><img src='assets/images/tab-icon-02.png' alt=''>Lunch</a></a></li>
                                   <li><a href='#tabs-3'><img src='assets/images/tab-icon-03.png' alt=''>Dinner</a></a></li>
                                 </ul>
                             </div>
                         </div>
                     </div>
                 </div>
                 <div class='col-lg-12'>
                     <section class='tabs-content'>
                         <article id='tabs-1'>
                             <div class='row'>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='left-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-01.png' alt=''>
                                                     <h4>Fresh Chicken Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>100 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-02.png' alt=''>
                                                     <h4>Orange Juice</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>85 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-03.png' alt=''>
                                                     <h4>Fruit Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>90 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='right-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-04.png' alt=''>
                                                     <h4>Eggs Omelette</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>65 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-05.png' alt=''>
                                                     <h4>Dollma Pire</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>50 rs</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-06.png' alt=''>
                                                     <h4>Omelette & Cheese</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>40 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
                         </article>  
                         <article id='tabs-2'>
                             <div class='row'>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='left-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-04.png' alt=''>
                                                     <h4>Eggs Omelette</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>140 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-05.png' alt=''>
                                                     <h4>Dollma Pire</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>180 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-06.png' alt=''>
                                                     <h4>Omelette & Cheese</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>220 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='right-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-01.png' alt=''>
                                                     <h4>Fresh Chicken Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>10 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-02.png' alt=''>
                                                     <h4>Orange Juice</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>20 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-03.png' alt=''>
                                                     <h4>Fruit Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>30 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
                         </article>  
                         <article id='tabs-3'>
                             <div class='row'>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='left-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-05.png' alt=''>
                                                     <h4>Eggs Omelette</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>140 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-03.png' alt=''>
                                                     <h4>Orange Juice</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>180 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-02.png' alt=''>
                                                     <h4>Fruit Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>100 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='right-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-06.png' alt=''>
                                                     <h4>Fresh Chicken Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>85 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-01.png' alt=''>
                                                     <h4>Dollma Pire</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>90rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-04.png' alt=''>
                                                     <h4>Omelette & Cheese</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>110 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>
                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
                         </article>   
                     </section>
                 </div>
             </div>
         </div>
     </div>
 </div>
</section>"
?>

                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-06.png' alt=''>
                                                     <h4>Omelette & Cheese</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>40 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
                         </article>  
                         <article id='tabs-2'>
                             <div class='row'>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='left-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-04.png' alt=''>
                                                     <h4>Eggs Omelette</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>140 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-05.png' alt=''>
                                                     <h4>Dollma Pire</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>180 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-06.png' alt=''>
                                                     <h4>Omelette & Cheese</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>220 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='right-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-01.png' alt=''>
                                                     <h4>Fresh Chicken Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>10 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-02.png' alt=''>
                                                     <h4>Orange Juice</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>20 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-03.png' alt=''>
                                                     <h4>Fruit Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>30 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
                         </article>  
                         <article id='tabs-3'>
                             <div class='row'>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='left-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-05.png' alt=''>
                                                     <h4>Eggs Omelette</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>140 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-03.png' alt=''>
                                                     <h4>Orange Juice</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>180 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-02.png' alt=''>
                                                     <h4>Fruit Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>100 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class='col-lg-6'>
                                     <div class='row'>
                                         <div class='right-list'>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-06.png' alt=''>
                                                     <h4>Fresh Chicken Salad</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>85 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-01.png' alt=''>
                                                     <h4>Dollma Pire</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>90rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                             <div class='col-lg-12'>
                                                 <div class='tab-item'>
                                                     <img src='assets/images/tab-item-04.png' alt=''>
                                                     <h4>Omelette & Cheese</h4>
                                                     <p>kv's cafe special for you.....................</p>
                                                     <div class='price'>
                                                         <h6>110 rs.</h6>
                                                         <form action='order.php' method='post'>
    <div class='price'>
        <h6>100 rs.</h6>
        <input type='hidden' name='item' value='Fresh Chicken Salad'>
        <input type='hidden' name='price' value='100'>
        <input type='number' name='quantity' value='1' min='1'>
        <button style:"background-color: orange;" type='submit' name='add_to_cart'>Add to Cart</button>
    </div>
</form>



                                                     </div>
                                                 </div>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
                         </article>   
                     </section>
                 </div>
             </div>
         </div>
     </div>
 </div>
</section>"
?>
</body>
</html>